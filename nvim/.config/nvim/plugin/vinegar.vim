...
function! s:escapedabs(first, last) abort
  let files = s:absolutes(a:first, a:last)
  return join(map(files, 's:fnameescape(v:val)'), ' ')
endfunction
...
function! s:setup_vinegar() abort
  ...
  nnoremap <buffer> . :<C-U> <C-R>=<SID>escapedabs(line('.'), line('.') - 1 + v:count1)<CR><Home>
  xnoremap <buffer> . <Esc>: <C-R>=<SID>escapedabs(line("'<"), line("'>"))<CR><Home>
